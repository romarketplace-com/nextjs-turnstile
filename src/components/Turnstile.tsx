/** biome-ignore-all lint/suspicious/noExplicitAny: we need to assign to window properties in these utilities and tests */
	...
	useEffect(() => {
		...
		if (
			container.querySelector(
				'iframe[src*="turnstile"], iframe[allow*="cross-origin"]',
			)
		) {
			return;
		}
		...
		return () => {
			...
			const isRealUnmount =
				!containerRef.current || !document.body.contains(containerRef.current);
		...
		callbackRefs.onAfterInteractive.current,
		callbackRefs.onBeforeInteractive.current,
		callbackRefs.onError.current,
		callbackRefs.onExpire.current,
		callbackRefs.onLoad.current,
		callbackRefs.onSuccess.current,
		callbackRefs.onTimeout.current,
		callbackRefs.onUnsupported.current,
	]);
